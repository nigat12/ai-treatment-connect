# -*- coding: utf-8 -*-
# --------------------------------------------------------------------------
# AI Cancer Information Assistant (Production Candidate - v2.3 Professional & Reliable)
# --------------------------------------------------------------------------
...
TRIAL_ACCEPTABLE_PHASES_STR = ['PHASE1|PHASE2', 'PHASE2', 'PHASE2|PHASE3', 'PHASE3', 'PHASE4', 'PHASE 1', 'PHASE 1B', 'PHASE 1/PHASE 2', 'EARLY PHASE 1', 'NOT APPLICABLE'] # Added 'NOT APPLICABLE' for some trials
...
def parse_time_to_months(time_str):
    ...
    match_months = re.search(r'(\d+(\.\d+)?)\s*months?', time_str_cleaned)
    if match_months: return float(match_months.group(1))
    match_years = re.search(r'(\d+(\.\d+)?)\s*years?', time_str_cleaned)
...
def load_and_preprocess_drug_data(csv_path):
    try:
        ...
        for col in DRUG_TEXT_COLUMNS_FOR_EMBEDDING:
...
def get_or_generate_drug_embeddings(_drug_df, _model, embeddings_path=DRUG_EMBEDDINGS_FILE):
    if os.path.exists(embeddings_path):
        ...
        except Exception as e: logger.warning(f"Error loading drug embeddings from cache: {e}. Regenerating.", exc_info=True)
    try:
        ...
        logger.info("Drug embeddings generated and saved.")
        return embeddings
    except Exception as e:
            ...
            def parse_excel_coords_robust(coord_str_val):
                ...
                try:
                    ...
                    if match_simple_tuple:
                        try: return [(float(match_simple_tuple.group(1)), float(match_simple_tuple.group(2)))]
                        except ValueError: pass
                    logger.debug(f"Could not parse coordinate string: '{str(coord_str_val)[:50]}...'")
                except Exception as parse_err: logger.warning(f"General error parsing coordinate string: '{str(coord_str_val)[:50]}...'. Error: {parse_err}")
                return []
...
def get_or_generate_trial_embeddings(_trial_df, _model, embeddings_path=TRIAL_EMBEDDINGS_FILE, map_path=TRIAL_INDEX_MAP_FILE):
    if os.path.exists(embeddings_path) and os.path.exists(map_path):
        ...
        except Exception as e: logger.warning(f"Error loading trial embeddings/map from cache: {e}. Regenerating.", exc_info=True)
    try:
        ...
        if not non_empty_mask.any(): return np.array([]), {}
...
try:
    if NOMINATIM_USER_AGENT == f"AI_Cancer_Info_Assistant_Prod/1.8 your_contact_email@example.com":
        st.sidebar.warning("**Geocoding Service Alert:** Update `NOMINATIM_USER_AGENT` for reliable geocoding.")
    ...
    if embedding_model_global and \
       ...
       df_trials_processed_global is not None and (trial_embeddings_array_global is not None or df_trials_processed_global.empty) and \
       (trial_index_map_global is not None or df_trials_processed_global.empty):
...
except RuntimeError: logger.critical("Startup halted by RuntimeError during data loading.", exc_info=False)
except Exception as e:
...
def _determine_top_n_results(scored_results_list, min_results=1, max_results=3, default_results=2):
    ...
    if count > 0 and scored_results_list[0]['semantic_similarity'] > 0.78: return min(count, max_results + 1)
    if count > (default_results - 1) and scored_results_list[default_results-1]['semantic_similarity'] > 0.68: return min(count, default_results)
    return min(count, min_results + 1)

def load_persistent_geocode_cache(filepath=GEOCODE_CACHE_FILE_PATH):
    if os.path.exists(filepath):
        try:
            with open(filepath, 'r') as f: return json.load(f)
        except json.JSONDecodeError: logger.warning(f"Geocode cache file '{filepath}' is corrupted.")
        except Exception as e: logger.warning(f"Error loading geocode cache '{filepath}': {e}.")
    return {}
...
def save_persistent_geocode_cache(cache_data, target_path=GEOCODE_CACHE_FILE_PATH, temp_path=TEMP_GEOCODE_CACHE_FILE_PATH):
    ...
    except Exception as e: logger.warning(f"Could not save geocode cache to '{target_path}': {e}", exc_info=True)
...
def find_drugs_tool(diagnosis: str, stage: str, biomarkers: str, line_of_therapy: str = None) -> str:
    ...
    Args:
        ...
        stage (str): The cancer stage.
        biomarkers (str): Known biomarkers or genetic mutations.
        line_of_therapy (str, optional): The line of therapy if known.

    Returns:
        str: A JSON string of potential drugs with details. The AI assistant MUST summarize this
             patient-friendly using specified Markdown and NOT show raw data or similarity scores.
    """
    ...
    if not query_text: return json.dumps({"error": "Please provide a diagnosis to search for drugs.", "drugs": []})
    try:
        ...
        logger.error(f"Error in find_drugs_tool embedding/similarity: {e}", exc_info=True)
        return json.dumps({"error": "Internal error searching for drugs.", "drugs": []})
    potential_results = []
    ...
    final_selected_results = potential_results[:_determine_top_n_results(potential_results)]
    if not final_selected_results: return json.dumps({"message": "No drug studies closely matched your criteria.", "drugs": []})
...
def find_clinical_trials_tool(diagnosis: str, stage: str, biomarkers: str, line_of_therapy: str = None, user_latitude: float = None, user_longitude: float = None, radius_miles: int = DEFAULT_SEARCH_RADIUS_MILES) -> str:
    ...
    Args:
        ...
        stage (str): Cancer stage.
        biomarkers (str): Biomarkers or genetic mutations.
        line_of_therapy (str, optional): Line of therapy if known.
        user_latitude (float, optional): User's latitude for location search.
        user_longitude (float, optional): User's longitude for location search.
        radius_miles (int): Search radius in miles if location is provided.

    Returns:
        str: A JSON string of trials with details. The AI assistant MUST summarize this
             patient-friendly using specified Markdown and NOT show raw data or similarity scores.
    """
    ...
    query_parts = [diagnosis, stage, biomarkers, line_of_therapy]
    query_text = " ".join(filter(None, query_parts)).strip().lower()
    if not query_text: return json.dumps({"error": "Please provide a diagnosis to search for clinical trials.", "trials": []})
    user_coordinates = (user_latitude, user_longitude) if user_latitude is not None and user_longitude is not None else None
    try: query_embedding = embedding_model_global.encode(query_text, convert_to_numpy=True)
    except Exception as e:
        logger.error(f"Error in find_clinical_trials_tool embedding: {e}", exc_info=True)
        return json.dumps({"error": "Internal error preparing trial search.", "trials": []})
    potential_results = []
    for original_df_idx in trial_index_map_global.keys():
        if original_df_idx not in df_trials_processed_global.index: continue
        row_data = df_trials_processed_global.loc[original_df_idx]
        embedding_idx = trial_index_map_global[original_df_idx]
        if embedding_idx >= len(trial_embeddings_array_global): continue
        ...
        if not (check_phases(trial_phases_str) and study_type_str == TRIAL_FILTER_STUDY_TYPE_VALUE.upper()): continue
        ...
        trial_details = {
            ...
            'status': row_data.get('Study Status', 'N/A'), 'phases': trial_phases_str,
            ...
            'semantic_similarity': round(float(similarity_score), 4), 'distance_miles': None,
                ...
                for site_coords_tuple in trial_site_coordinates_list:
                    if isinstance(site_coords_tuple, tuple) and len(site_coords_tuple) == 2:
                        try:
                            ...
                            if current_distance < min_distance_miles: min_distance_miles = current_distance
                        except Exception: pass # Ignore individual distance calculation errors
        ...
        else: potential_results.append(trial_details)
    if user_coordinates: potential_results.sort(key=lambda x: (sort_key_with_none(x['distance_miles'], False), -x['semantic_similarity']))
    else: potential_results.sort(key=lambda x: x['semantic_similarity'], reverse=True)
    final_selected_results = potential_results[:_determine_top_n_results(potential_results)]
    if not final_selected_results: return json.dumps({"message": "No clinical trials closely matched your criteria or location.", "trials": []})
...
def zip_to_coordinates_tool(zip_code: str, country_code: str = "US") -> str:
    ...
    Args:
        ...
        country_code (str, optional): Country code for the zip code. Defaults to "US".

    Returns:
        str: A JSON string with 'latitude', 'longitude', and 'status'. Used for location-specific searches.
    """
    ...
    try:
        ...
        return json.dumps({"error": f"{nominatim_warning_message}Could not find location for zip: {zip_code}. Check and try again.", "status": "error_not_found"})
    except (GeocoderTimedOut, GeocoderUnavailable) as ge_service_error:
        return json.dumps({"error": f"{nominatim_warning_message}Location service unavailable. Try again later.", "status": "error_service_unavailable"})
    except Exception as general_ge_error:
        return json.dumps({"error": f"{nominatim_warning_message}Unexpected error geocoding zip: {zip_code}.", "status": "error_unknown"})
...
def generate_follow_up_suggestions(user_query: str, ai_main_response: str, llm_client: ChatAnthropic) -> list[str]:
    ...
    ai_main_response = str(ai_main_response) if ai_main_response is not None else ""
    max_len = 500
    ...
    suggestion_prompt_messages = [
        ...
        HumanMessage(content=f"User's last query: \"{truncated_user_query}\"\n\nAI Assistant's last response: \"{truncated_ai_response}\"\n\nGenerate 2-3 distinct, relevant follow-up questions based on this exchange. Ensure questions are actionable, directly related to the information just provided or logical next steps. Format as a JSON list of strings under a 'suggestions' key.")
    ]
    try:
        ...
        if json_match:
            ...
            try:
                ...
                if isinstance(suggestions_data.get("suggestions"), list):
                    valid_suggestions = [str(s).strip() for s in suggestions_data["suggestions"] if str(s).strip() and len(str(s).strip()) > 7 and str(s).strip().endswith("?")]
                    return valid_suggestions[:3]
            except json.JSONDecodeError: logger.warning(f"Could not decode JSON for suggestions: {json_str}")
        lines = [line.strip() for line in content.split('\n') if line.strip()]
        parsed_lines = [re.sub(r"^\s*[\d\.\-\*]+\s*", "", line).strip().replace("\"", "") for line in lines]
        valid_fallback = [s for s in parsed_lines if s and len(s) > 10 and s.endswith("?")]
        if valid_fallback: return valid_fallback[:3]
    except Exception as e: logger.error(f"Error generating follow-up suggestions: {e}", exc_info=True)
    return []
...
"""
    prompt_template = ChatPromptTemplate.from_messages(
        [
            ...
            MessagesPlaceholder(variable_name="agent_scratchpad"), # Corrected
        ]
    )
    llm = ChatAnthropic(
        model=CLAUDE_MODEL_NAME,
        temperature=0.05, api_key=ANTHROPIC_API_KEY,
        max_tokens_to_sample=4096, default_request_timeout=90.0
    )
    ...
    agent_executor = AgentExecutor(
        ...
        handle_parsing_errors=lambda e: f"I apologize, there was an issue processing the information. The error was: {str(e)[:200]}. Could you please rephrase or try again?",
        max_iterations=8, return_intermediate_steps=False,
...
if "current_input_from_suggestion" not in st.session_state: st.session_state.current_input_from_suggestion = None
# For checkboxes in sidebar, initialize in session_state if they don't exist
if "show_drug_emb_prog" not in st.session_state: st.session_state.show_drug_emb_prog = False
if "show_trial_emb_prog" not in st.session_state: st.session_state.show_trial_emb_prog = False
if "agent_verbose_cb" not in st.session_state: st.session_state.agent_verbose_cb = False
...
with st.sidebar:
    ...
    if st.button("🔄 Start New Conversation", use_container_width=True, type="primary", help="Clears the current chat and starts fresh."):
        ...
        st.session_state.current_input_from_suggestion = None
        if DATA_LOADED_SUCCESSFULLY:
            st.session_state.agent_executor, st.session_state.llm_client = get_langchain_agent_executor_and_llm()
    ...
    st.session_state.show_drug_emb_prog = st.checkbox("Show drug embedding progress", value=st.session_state.show_drug_emb_prog, key="drug_emb_prog_cb_sidebar")
    st.session_state.show_trial_emb_prog = st.checkbox("Show trial embedding progress", value=st.session_state.show_trial_emb_prog, key="trial_emb_prog_cb_sidebar")
    st.session_state.agent_verbose_cb = st.checkbox("Show agent thought process (verbose)", value=st.session_state.agent_verbose_cb, key="agent_verbose_cb_sidebar")
    st.markdown("---")
    st.subheader("Important Note")
    st.info("This AI provides information for learning. It is **not a substitute for medical advice.** Always consult your healthcare team.")
...
if st.session_state.current_input_from_suggestion:
    prompt_to_process = st.session_state.current_input_from_suggestion
    st.session_state.current_input_from_suggestion = None
    st.session_state.follow_up_suggestions_list = []

user_typed_prompt_str = st.chat_input("Ask me about cancer treatments, drugs, or trials...", key="main_chat_input_v6", disabled=(prompt_to_process is not None))

if not prompt_to_process and user_typed_prompt_str:
...
if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
    ...
    try:
        agent_invocation_input = {"input": last_user_prompt, "chat_history": st.session_state.chat_history_for_agent}
        if 'agent_executor' not in st.session_state or 'llm_client' not in st.session_state:
             st.session_state.agent_executor, st.session_state.llm_client = get_langchain_agent_executor_and_llm()
        agent_response_data = st.session_state.agent_executor.invoke(agent_invocation_input)
        raw_ai_output_content = None
        if isinstance(agent_response_data, dict) and "output" in agent_response_data: raw_ai_output_content = agent_response_data["output"]
        elif isinstance(agent_response_data, str): raw_ai_output_content = agent_response_data
        else:
            logger.error(f"Unexpected agent output structure: {type(agent_response_data)} - {str(agent_response_data)[:500]}")
            raw_ai_output_content = "I apologize, I encountered an issue formulating a complete response."
        if isinstance(raw_ai_output_content, list) and len(raw_ai_output_content) == 1 and \
           isinstance(raw_ai_output_content[0], dict) and 'text' in raw_ai_output_content[0]:
            final_assistant_response_str = str(raw_ai_output_content[0]['text'])
        else: final_assistant_response_str = str(raw_ai_output_content)
        ...
        final_assistant_response_str = (f"I'm truly sorry, but I encountered a significant technical issue: {error_detail_snippet}... "
                                        "The team has been notified. Please try again in a little while, or rephrase your question.")
        st.session_state.chat_history_for_agent.append(AIMessage(content=f"System error: {final_assistant_response_str}"))
    ...
    if final_assistant_response_str and "technical issue" not in final_assistant_response_str.lower() and "error" not in final_assistant_response_str.lower():
        st.session_state.follow_up_suggestions_list = generate_follow_up_suggestions(
            user_query=last_user_prompt, ai_main_response=final_assistant_response_str,
            llm_client=st.session_state.llm_client
        )
    st.rerun()

if st.session_state.follow_up_suggestions_list and st.session_state.messages:
    st.markdown("---")
    ...
    if num_suggestions_to_display > 0:
        ...
        for idx, suggestion_item_text in enumerate(st.session_state.follow_up_suggestions_list[:num_suggestions_to_display]):
            ...
            if suggestion_columns[idx].button(f"{suggestion_item_text}", key=button_key_str, use_container_width=True, help=f"Click to ask: {suggestion_item_text}"):
                st.session_state.current_input_from_suggestion = suggestion_item_text
                logger.info(f"Suggestion clicked: '{suggestion_item_text}'")
                st.rerun()

if not st.session_state.messages and DATA_LOADED_SUCCESSFULLY:
    initial_greeting_text = ("Hello! I'm an AI assistant here to help you explore information about cancer treatments, including "
...
if not DATA_LOADED_SUCCESSFULLY and not st.session_state.messages:
    st.error("The Cancer Information Assistant could not start due to data loading issues. Please check error messages and application setup. Check logs for details.")
